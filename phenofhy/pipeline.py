...
def _download_metadata_files() -> Dict[str, Path]:
    ...
    try:
        cfg_target = CONFIG_PATH  # for clearer error messages
...
def field_list(
    ...
    # 1) Load config
    try:
        cfg_target = CONFIG_PATH
        config_path = _load_or_download_file(cfg_target, _get_config_file_id(), "Config file", config_mode=True)
        with open(config_path, "r") as f:
            config = json.load(f)
    except Exception as e:
