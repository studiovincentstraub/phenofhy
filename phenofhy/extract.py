...
def _get_config_file_id() -> str | None:
    if CONFIG_PATH.exists():
        return None

    return find_latest_dx_file_id(
        name_pattern=CONFIG_FILENAME,
        folder=REMOTE_CONFIG_DIR,
    )
...
def load_or_download_file(file_path: Path, file_id: Path, description: str = "", validate_json=False):
    ...
    if file_id is None:
        raise FileNotFoundError(
            ...
            "and no remote file ID was supplied."
        )
...
def run_extraction(
    ...
    *,
    ...
    config_path = load_or_download_file(CONFIG_PATH, _get_config_file_id(), "Config file", validate_json=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cohort", type=str, default="FULL_SAMPLE")
